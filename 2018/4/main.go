...
func main() {
	...
	for _, p := range parts {
		...
		minute := strings.Split(ip[1][0:len(ip[1])-1], ":")
		intPart, err := strconv.ParseInt(minute[1], 10, 64)
		if err != nil {
			fmt.Printf("Failed to parse %q", p)
			return
		}
		if ip[2] == "falls" {
			lastMinute = int(intPart)
		}
		if ip[2] == "wakes" {
			for i := lastMinute; i < int(intPart); i++ {
				guards[guard][i] += 1
			}
		}
	}
...
var input = `[1518-05-12 00:39] wakes up
