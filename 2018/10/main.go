...
func main() {
	...
	dots := make([]*dot, 0, len(parts))
	for _, p := range parts {
		x, _ := strconv.Atoi(strings.Trim(p[10:16], " "))
		y, _ := strconv.Atoi(strings.Trim(p[18:24], " "))
		xVel, _ := strconv.Atoi(strings.Trim(p[36:38], " "))
		yVel, _ := strconv.Atoi(strings.Trim(p[39:42], " "))
		dots = append(dots, &dot{x, y, xVel, yVel})
	}
...
var input = `position=<-30580, -10081> velocity=< 3,  1>
